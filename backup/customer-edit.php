...
function mapPageTypeToDb(string $type): string {
    switch ($type) {
        case 'pawn':
            return 'PawnBroking';
        case 'chit':
...
function mapDbTypeToPage(string $type): string {
    switch ($type) {
        case 'PawnBroking':
            return 'pawn';
        case 'PawnChits':
...
function extractNoteValue(string $notes, string $label, string $default = ''): string {
    $lines = preg_split('/\r\n|\r|\n/', $notes);
    foreach ($lines as $line) {
        $prefix = $label . ':';
        if (stripos(trim($line), $prefix) === 0) {
            return trim(substr(trim($line), strlen($prefix)));
        }
    }
    return $default;
}
...
$sql = "SELECT * FROM customers WHERE id = ? AND business_id = ? AND is_active = 1 LIMIT 1";
$stmt = $conn->prepare($sql);
...
if (!$customer) {
    die('Customer not found or inactive.');
}

$customerType = $_GET['type'] ?? $_POST['page_customer_type'] ?? mapDbTypeToPage($customer['customer_type'] ?? 'Billing');
$validTypes = ['billing', 'pawn', 'chit'];

if (!in_array($customerType, $validTypes, true)) {
    $customerType = mapDbTypeToPage($customer['customer_type'] ?? 'Billing');
}
...
|--------------------------------------------------------------------------
*/
$existingNotes = (string)($customer['notes'] ?? '');

$existingShopName          = extractNoteValue($existingNotes, 'Shop/Business Name');
$existingCustomerCategory  = strtolower(extractNoteValue($existingNotes, 'Customer Category', 'retail'));
$existingZoneId            = (int)extractNoteValue($existingNotes, 'Zone ID', '0');
$existingAssignedArea      = extractNoteValue($existingNotes, 'Assigned Area');
$existingAssignedLinemanId = (int)extractNoteValue($existingNotes, 'Assigned Lineman ID', '0');
$existingPaymentTerms      = extractNoteValue($existingNotes, 'Payment Terms', 'cash');
$existingCreditLimit       = str_replace(['₹', ','], '', extractNoteValue($existingNotes, 'Credit Limit', '0'));
$existingOnlyNotes         = extractNoteValue($existingNotes, 'Notes');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_customer') {
    ...
    $customerTypeDb = mapPageTypeToDb($customerType);
    ...
    /*
    |--------------------------------------------------------------------------
    | Duplicate Mobile Check Except Current Customer
    |--------------------------------------------------------------------------
    */
    if (empty($errors)) {
        ...
        if ($checkStmt) {
            $checkStmt->bind_param('isi', $businessId, $customerContact, $customerId);
            $checkStmt->execute();
            $checkResult = $checkStmt->get_result();

            if ($checkResult && $checkResult->num_rows > 0) {
                $errors[] = 'Another customer with this contact number already exists!';
            }
    ...
    if (empty($errors)) {
        ...
        if (!$stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $customer['customer_type']     = $customerTypeDb;
                $customer['customer_name']     = $customerName;
                $customer['mobile']            = $customerContact;
                $customer['alternate_mobile']  = $alternateContact;
                $customer['email']             = $email;
                $customer['gstin']             = $gstNumber;
                $customer['address_line1']     = $shopLocation;
...
|--------------------------------------------------------------------------
*/
$currentNotes = (string)($customer['notes'] ?? '');

$postShopName          = $_POST['shop_name'] ?? extractNoteValue($currentNotes, 'Shop/Business Name');
$postCustomerName      = $_POST['customer_name'] ?? ($customer['customer_name'] ?? '');
$postCustomerContact   = $_POST['customer_contact'] ?? ($customer['mobile'] ?? '');
$postAlternateContact  = $_POST['alternate_contact'] ?? ($customer['alternate_mobile'] ?? '');
$postShopLocation      = $_POST['shop_location'] ?? ($customer['address_line1'] ?? '');
$postGstNumber         = $_POST['gst_number'] ?? ($customer['gstin'] ?? '');
$postEmail             = $_POST['email'] ?? ($customer['email'] ?? '');
$postCustomerCategory  = $_POST['customer_category'] ?? strtolower(extractNoteValue($currentNotes, 'Customer Category', 'retail'));
$postAssignedLinemanId = (int)($_POST['assigned_lineman_id'] ?? extractNoteValue($currentNotes, 'Assigned Lineman ID', '0'));
$postAssignedArea      = $_POST['assigned_area'] ?? extractNoteValue($currentNotes, 'Assigned Area');
$postPaymentTerms      = $_POST['payment_terms'] ?? extractNoteValue($currentNotes, 'Payment Terms', 'cash');
$postCreditLimit       = $_POST['credit_limit'] ?? str_replace(['₹', ','], '', extractNoteValue($currentNotes, 'Credit Limit', '0'));
$postNotes             = $_POST['notes'] ?? extractNoteValue($currentNotes, 'Notes');
$postZoneId            = (int)($_POST['zone_id'] ?? extractNoteValue($currentNotes, 'Zone ID', '0'));

if ($postCustomerCategory === '') {
    $postCustomerCategory = 'retail';
}
if ($postPaymentTerms === '') {
    $postPaymentTerms = 'cash';
}
?>
<!doctype html>
<html lang="en">
            ...
            <div class="container-fluid">
                        ...
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <div>
                                ...
                                <div class="text-muted">
                                    Update customer account - <?php echo h($customer['customer_code'] ?? ''); ?>
                                </div>
                            </div>
                            <div>
                ...
                <form method="POST" action="" id="customerForm">
                    ...
                    <div class="text-end mb-4">
                        <a href="customers.php?type=<?php echo h($customerType); ?>" class="btn btn-outline-secondary me-2">Cancel</a>
