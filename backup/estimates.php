...
|--------------------------------------------------------------------------
| New Estimate and Edit Estimate should open the estimate form page, not the
| normal billing create page. Change this filename here only if your estimate
| form page has a different name.
*/
$estimateFormPage = 'estimate.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_estimate'])) {
    ...
    $reason = trim((string)($_POST['cancel_reason'] ?? 'Deleted from estimates list'));

    if ($saleId <= 0) {
        $error = 'Invalid estimate selected.';
    } else {
        $conn->begin_transaction();
        try {
            $stmt = $conn->prepare("SELECT id, bill_no, status FROM sales WHERE id = ? AND business_id = ? LIMIT 1 FOR UPDATE");
            if (!$stmt) {
                throw new Exception('Failed to prepare estimate check.');
            }
            ...
            if (!$stmt) {
                throw new Exception('Failed to prepare estimate items.');
            }
            $stmt->bind_param('ii', $saleId, $businessId);
            $stmt->execute();
            $res = $stmt->get_result();
            ...
            if (!$stmt) {
                throw new Exception('Failed to prepare estimate cancellation.');
            }
            ...
            foreach ($items as $item) {
                ...
                if ($hasStockMovementTable) {
                    $remarksText = 'Estimate cancelled / stock restored for estimate ' . (string)$sale['bill_no'];
            ...
            addAuditLogSafe($conn, $businessId, $userId, 'Estimates', 'DELETE', $saleId, 'Estimate ' . (string)$sale['bill_no'] . ' cancelled from estimates list. Reason: ' . $reason);
            $conn->commit();
            $success = 'Estimate cancelled successfully and stock restored.';
        } catch (Throwable $e) {
            $conn->rollback();
            $error = $e->getMessage();
        }
...
$where = "s.business_id = ? AND s.bill_type = 'Estimate'";
$params = [$businessId];
$types = 'i';
        ...
        <div class="page-content">
            <div class="container-fluid">
                    ...
                    <div class="col-12">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <div>
                                ...
                                <div class="text-muted">View, print, edit and cancel estimate bills</div>
                            </div>
                            <a href="<?php echo h($estimateFormPage); ?>" class="btn btn-primary"><i class="fas fa-plus me-1"></i> New Estimate</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-md-4">
                        <div class="card summary-card h-100"><div class="card-body"><div class="summary-label">Estimate Total</div><h3 class="summary-value">₹<?php echo money($totals['grand_total']); ?></h3></div></div>
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="card-body">
                        <form method="get" class="row g-3 align-items-end">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" name="search" class="form-control" placeholder="Estimate no / customer / mobile / reference" value="<?php echo h($search); ?>">
                            </div>
                            <div class="col-md-2 d-flex gap-2">
                                <button type="submit" class="btn btn-primary w-100"><i class="fas fa-search me-1"></i> Search</button>
                                <a href="estimates.php" class="btn btn-light border"><i class="fas fa-redo"></i></a>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                            <h5 class="mb-0">Estimate Bills</h5>
                                    ...
                                    <tr class="<?php echo $isCancelled ? 'table-danger' : ''; ?>">
                                        ...
                                        <td>
                                            <div class="action-btns">
                                                <a href="estimates.php?<?php echo h(http_build_query(array_merge($_GET, ['view_id' => $row['id']]))); ?>" class="btn btn-sm btn-outline-info" title="View"><i class="fas fa-eye"></i></a>
                                                <a href="estimate-print.php?id=<?php echo (int)$row['id']; ?>" target="_blank" class="btn btn-sm btn-outline-secondary" title="Print"><i class="fas fa-print"></i></a>
                                                <?php if (!$isCancelled): ?>
                                                    <a href="<?php echo h($estimateFormPage . '?edit_id=' . (int)$row['id']); ?>" class="btn btn-sm btn-outline-primary" title="Edit"><i class="fas fa-edit"></i></a>
                                                    <form method="post" class="d-inline" onsubmit="return confirm('Are you sure you want to cancel this estimate? Stock will be restored.');">
                                                        ...
                                                        <button type="submit" name="delete_estimate" class="btn btn-sm btn-outline-danger" title="Delete"><i class="fas fa-trash"></i></button>
                                                    </form>
                                                <?php else: ?>
                    ...
                    <div class="modal-dialog modal-xl modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="saleViewModalLabel">Estimate Details - <?php echo h($viewSale['bill_no']); ?></h5>
                                <a href="estimates.php?<?php $tmp = $_GET; unset($tmp['view_id']); echo h(http_build_query($tmp)); ?>" class="btn-close" aria-label="Close"></a>
                            </div>
                            <div class="modal-body">
                                <div class="row g-3 mb-3">
                                    <div class="col-md-3"><strong>Estimate No:</strong><br><?php echo h($viewSale['bill_no']); ?></div>
                            ...
                            <div class="modal-footer">
                                <a href="estimate-print.php?id=<?php echo (int)$viewSale['id']; ?>" target="_blank" class="btn btn-secondary"><i class="fas fa-print me-1"></i> Print</a>
                                <?php if ((string)$viewSale['status'] !== 'Cancelled'): ?>
                                    <a href="<?php echo h($estimateFormPage . '?edit_id=' . (int)$viewSale['id']); ?>" class="btn btn-primary"><i class="fas fa-edit me-1"></i> Edit</a>
                                <?php endif; ?>
                                <a href="estimates.php?<?php $tmp = $_GET; unset($tmp['view_id']); echo h(http_build_query($tmp)); ?>" class="btn btn-light border">Close</a>
                            </div>
                        </div>
                    </div>
                </div>
