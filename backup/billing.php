...
function generateBillNo(mysqli $conn, int $businessId, string $prefix = 'INV'): string
{
    $sql = "SELECT COUNT(*) AS cnt FROM sales WHERE business_id = ?";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
...
$productsHasPurchaseRate    = hasColumn($conn, 'products', 'purchase_rate');
...
$categoriesHasBusinessId    = hasColumn($conn, 'product_categories', 'business_id');
...
$companySettingsHasBusinessId    = $hasCompanySettingsTable && hasColumn($conn, 'company_settings', 'business_id');
$companySettingsHasInvoicePrefix = $hasCompanySettingsTable && hasColumn($conn, 'company_settings', 'invoice_prefix');
...
if ($hasCompanySettingsTable && $companySettingsHasBusinessId && $companySettingsHasInvoicePrefix) {
    $stmt = $conn->prepare("SELECT invoice_prefix FROM company_settings WHERE business_id = ? LIMIT 1");
...
if ($customersHasMobile) {
    $sql .= ", mobile";
}

if ($customersHasGstin) {
    $sql .= ", gstin";
}
...
$sql = "
    SELECT
        ...
        " . ($productsHasPurchaseRate ? "IFNULL(p.purchase_rate,0) AS purchase_rate," : "0 AS purchase_rate,") . "
...
$defaultBillNo = generateBillNo($conn, $businessId, $invoicePrefix);
    ...
    if ($customerId <= 0 && $quickCustomerName === '') {
        ...
        if ($error === '') {
            ...
            try {
                ...
                $billNo = generateBillNo($conn, $businessId, $invoicePrefix);
                ...
                if ($hasSalePaymentsTable && $salePaymentsHasSaleId && $salePaymentsHasMethodId && $salePaymentsHasAmount) {
                    foreach ($paymentRows as $prow) {
                        ...
                        if (!$stmt) {
                            throw new Exception('Failed to prepare sale payment insert.');
                        }
                ...
                header('Location: sales-list.php?msg=created');
                exit;
            } catch (Throwable $e) {
                $conn->rollback();
...
<div class="billing-page-wrapper">
    <div class="container-fluid px-0">
        ...
        <div class="billing-header">
            ...
            <button type="button" class="btn btn-danger billing-close-btn" onclick="window.location='sales-list.php'">Close</button>
        </div>
                        ...
                        <div class="card-body">
                            <div class="row g-2">
                                <div class="col-md-3">
                                    ...
                                    <input type="text" class="form-control" value="<?php echo h($defaultBillNo); ?>" readonly>
                                </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body p-2">
                            ...
                            <small class="compact-note d-block mt-1">
                                Example: Cash ₹5000 + UPI ₹3000 + Card ₹2000
                            </small>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card sticky-summary">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mt-3 d-grid gap-2">
                                <button type="submit" name="save_bill" value="1" class="btn btn-primary">Save Bill</button>
                                <button type="button" class="btn btn-secondary" onclick="window.location='billing.php'">Reset</button>
                                <button type="button" class="btn btn-danger" onclick="window.location='sales-list.php'">Close</button>
                            </div>
                        </div>
                    </div>
                </div>
