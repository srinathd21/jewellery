...
function bindStmt(mysqli_stmt $stmt, string $types, array &$values): bool
{
    if (strlen($types) !== count($values)) {
        throw new Exception('Bind parameter mismatch. Types: ' . strlen($types) . ', Values: ' . count($values));
    }
...
function addAuditLogSafe(mysqli $conn, ?int $businessId, ?int $userId, string $module, string $action, int $refId, string $desc): void
{
    ...
    $values = [$businessId, $userId, $module, $action, $refId, $desc, $ip, $ua];
    bindStmt($stmt, 'iississs', $values);
    $stmt->execute();
    $stmt->close();
}

function generateEstimateNo(mysqli $conn, int $businessId, string $prefix = 'EST'): string
{
    $next = 1;

    $stmt = $conn->prepare("SELECT COUNT(*) AS cnt FROM sales WHERE business_id = ? AND bill_type = 'Estimate'");
    if ($stmt) {
...
function generateCustomerCode(mysqli $conn, int $businessId): string
{
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) AS cnt FROM customers WHERE business_id = ?");
    if ($stmt) {
        $stmt->bind_param('i', $businessId);
        $stmt->execute();
        $res = $stmt->get_result();
...
function ensureProductStockRow(mysqli $conn, int $businessId, int $productId): bool
{
    ...
    $stmt = $conn->prepare("
        ...
        (business_id, product_id, opening_qty, opening_weight, in_qty, in_weight, out_qty, out_weight, closing_qty, closing_weight, created_at, updated_at)
...
if ($hasCompanySettingsTable && hasColumn($conn, 'company_settings', 'estimate_prefix')) {
    $stmt = $conn->prepare("SELECT estimate_prefix FROM company_settings WHERE business_id = ? LIMIT 1");
    if ($stmt) {
        $stmt->bind_param('i', $businessId);
        $stmt->execute();
        $res = $stmt->get_result();
        ...
        if (!empty($row['estimate_prefix'])) {
            $estimatePrefix = (string)$row['estimate_prefix'];
        }
    }
}
...
$customers = [];
$stmt = $conn->prepare("
    SELECT id, customer_code, customer_name, mobile, gstin
    FROM customers
...
$stmt = $conn->prepare("
    SELECT
        ...
        IFNULL(p.design_name,'') AS design_name,
        IFNULL(p.purity,'925') AS purity,
        IFNULL(p.unit,'pcs') AS unit,
        IFNULL(p.gross_weight,0) AS gross_weight,
        IFNULL(p.less_weight,0) AS less_weight,
        IFNULL(p.net_weight,0) AS net_weight,
        IFNULL(p.making_charge_type,'fixed') AS making_charge_type,
        IFNULL(p.making_charge,0) AS making_charge,
        IFNULL(p.wastage_percent,0) AS wastage_percent,
        IFNULL(p.stone_charge,0) AS stone_charge,
        IFNULL(p.purchase_rate,0) AS purchase_rate,
        IFNULL(p.sale_rate,0) AS sale_rate,
        IFNULL(p.current_stock_qty,0) AS current_stock_qty,
        IFNULL(pc.category_name,'') AS category_name,
        IFNULL(pc.hsn_code,'') AS hsn_code,
        IFNULL(pc.gst_percent,3) AS gst_percent
    FROM products p
...
$defaultEstimateNo = generateEstimateNo($conn, $businessId, $estimatePrefix);
$billDate = (string)($_POST['bill_date'] ?? date('Y-m-d'));
$billTime = (string)($_POST['bill_time'] ?? date('H:i'));
...
$splitPaymentMethodIds = $_POST['split_payment_method_id'] ?? [0];
$splitPaymentAmounts = $_POST['split_payment_amount'] ?? ['0'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_estimate'])) {
    ...
    if ($customerId <= 0 && $quickCustomerName === '') {
        ...
        if ($error === '') {
            ...
            $primaryPaymentMethodId = !empty($paymentRows) ? (int)$paymentRows[0]['payment_method_id'] : null;
            ...
            if ($error === '') {
                ...
                try {
                    if ($customerId <= 0 && $quickCustomerName !== '') {
                        ...
                        $values = [$businessId, $customerCode, $quickCustomerName, $quickCustomerMobile, $quickCustomerGstin];
                        bindStmt($stmt, 'issss', $values);

                        if (!$stmt->execute()) {
                            $stmtError = $stmt->error;
                            $stmt->close();
                            throw new Exception('Failed to create quick customer: ' . $stmtError);
                        }
                    ...
                    if ($customerId > 0) {
                        $stmt = $conn->prepare("SELECT customer_name, mobile FROM customers WHERE id = ? AND business_id = ? LIMIT 1");
                    ...
                    $billType = 'Estimate';
                    $estimateNo = generateEstimateNo($conn, $businessId, $estimatePrefix);

                    $stmt = $conn->prepare("
                        INSERT INTO sales
                        (business_id, bill_no, bill_date, bill_time, customer_id, customer_name, customer_mobile, bill_type, payment_method_id, payment_reference, subtotal, discount_amount, taxable_amount, cgst_amount, sgst_amount, igst_amount, round_off, grand_total, paid_amount, balance_amount, payment_status, notes, status, created_by, created_at, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active', ?, NOW(), NOW())
                    ");

                    if (!$stmt) {
                        throw new Exception('Failed to prepare estimate insert: ' . $conn->error);
                    }
                    ...
                    bindStmt($stmt, 'isssisssisddddddddddssi', $values);

                    if (!$stmt->execute()) {
                        $stmtError = $stmt->error;
                        $stmt->close();
                        throw new Exception('Failed to save estimate: ' . $stmtError);
                    }
                    ...
                    foreach ($lineItems as $item) {
                        ...
                        if (!$stmt) {
                            throw new Exception('Failed to prepare estimate item insert: ' . $conn->error);
                        }
                        ...
                        bindStmt($stmt, 'iiissssssddddddsdddddddddd', $values);

                        if (!$stmt->execute()) {
                            $stmtError = $stmt->error;
                            $stmt->close();
                            throw new Exception('Failed to save estimate item: ' . $stmtError);
                        }
                        ...
                        if ($productsHasCurrentStockQty) {
                            ...
                            if (!$stmt) {
                                throw new Exception('Failed to prepare product stock update.');
                            }

                            $values = [$item['qty'], $item['product_id'], $businessId];
                            bindStmt($stmt, 'dii', $values);

                            if (!$stmt->execute()) {
                                $stmtError = $stmt->error;
                                $stmt->close();
                                throw new Exception('Failed to update product stock: ' . $stmtError);
                            }
                        ...
                        if ($hasProductStockTable && $productStockHasProductId) {
                            ...
                            if (!$stmt) {
                                throw new Exception('Failed to prepare product_stock update.');
                            }
                        ...
                        if ($hasStockMovementTable) {
                            $stmt = $conn->prepare("
                                ...
                                VALUES (?, NOW(), ?, 'Estimate', 'sales', ?, 0, ?, 0, ?, ?, ?, NOW())
                            ");

                            if ($stmt) {
                                $remarksText = 'Estimate ' . $estimateNo;
                                $values = [
                                    $businessId,
                                    $item['product_id'],
                                    $saleId,
                    ...
                    if ($hasSalePaymentsTable && !empty($paymentRows)) {
                        foreach ($paymentRows as $prow) {
                            $insertCols = ['sale_id', 'payment_method_id', 'amount'];
                            $insertVals = ['?', '?', '?'];
                            $types = 'iid';
                            $values = [$saleId, (int)$prow['payment_method_id'], (float)$prow['amount']];
                            ...
                            if (hasColumn($conn, 'sale_payments', 'reference_no')) {
                                $insertCols[] = 'reference_no';
                                $insertVals[] = '?';
                                $types .= 's';
                                $values[] = (string)$prow['reference'];
                            }

                            if (hasColumn($conn, 'sale_payments', 'created_by')) {
                            ...
                            if (!$stmt) {
                                throw new Exception('Failed to prepare estimate payment insert: ' . $conn->error);
                            }

                            bindStmt($stmt, $types, $values);

                            if (!$stmt->execute()) {
                                $stmtError = $stmt->error;
                                $stmt->close();
                                throw new Exception('Failed to save payment split: ' . $stmtError);
                            }
                    ...
                    header('Location: estimates.php?msg=created');
                    exit;
                } catch (Throwable $e) {
                    $conn->rollback();
...
<div class="billing-page-wrapper">
    <div class="container-fluid px-0">
        ...
        <div class="billing-header">
            <h1 class="billing-title">Estimate</h1>
            <button type="button" class="btn btn-danger billing-close-btn" onclick="window.location='estimates.php'">Close</button>
        </div>
        ...
        <form method="post" id="billingForm" autocomplete="off">
            <div class="row g-2">
                <div class="col-xl-9 col-lg-8">
                        ...
                        <div class="card-body">
                            <div class="row g-2">
                                <div class="col-md-3">
                                    ...
                                    <input type="text" class="form-control" value="<?php echo h($defaultEstimateNo); ?>" readonly>
                                </div>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="text" class="form-control" value="Estimate" readonly>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="notes" class="form-control" value="<?php echo h($notes); ?>" placeholder="Estimate notes">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h5 class="mb-0">Estimate Items</h5>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body p-2">
                            ...
                            <small class="compact-note d-block mt-1">
                                Optional for estimate. Example: Cash ₹5000 + UPI ₹3000
                            </small>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card sticky-summary">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mt-3 d-grid gap-2">
                                <button type="submit" name="save_estimate" value="1" class="btn btn-primary">Save Estimate</button>
                                <button type="button" class="btn btn-secondary" onclick="window.location='estimate.php'">Reset</button>
                                <button type="button" class="btn btn-danger" onclick="window.location='estimates.php'">Close</button>
                            </div>
                        </div>
                    </div>
                </div>
