...
if ($stmt) {
    ...
    while ($res && ($row = $res->fetch_assoc())) {
        $formItems[] = [
            ...
            'less_weight'      => qtyf($row['less_weight'] ?? 0),
            'net_weight'       => qtyf($row['net_weight'] ?? 0),
            'rate_per_gram'    => moneyf($row['rate_per_gram'] ?? 0),
            'making_charge'    => moneyf($row['making_charge'] ?? 0),
            'stone_charge'     => moneyf($row['stone_charge'] ?? 0),
            'item_amount'      => moneyf($row['item_amount'] ?? 0),
            'discount_amount'  => moneyf($row['discount_amount'] ?? 0),
            'taxable_amount'   => moneyf($row['taxable_amount'] ?? 0),
            'gst_percent'      => moneyf($row['gst_percent'] ?? 3),
            'gst_amount'       => moneyf($row['gst_amount'] ?? 0),
            'total_amount'     => moneyf($row['total_amount'] ?? 0),
        ];
    }
    ...
    if ($purchaseNo === '') {
        ...
        if ($error === '') {
            ...
            try {
                ...
                $sql = "SELECT product_id, qty, net_weight FROM purchase_items WHERE purchase_id = ?";
                ...
                foreach ($oldItems as $old) {
                    ...
                    $oldNetWeight = (float)($old['net_weight'] ?? 0);

                    if ($oldProductId > 0) {
                        updateProductStockQty($conn, $oldProductId, $businessId, $prdHasBusinessId, $prdHasCurrentStockQty, -$oldQty);

                        updateProductStockTable(
                            $conn,
                            $oldProductId,
                            $businessId,
                            $productStockExists,
                            $productStockHasBiz,
                            $productStockHasProduct,
                            $productStockHasInQty,
                            $productStockHasInWt,
                            $productStockHasCloseQty,
                            $productStockHasCloseWt,
                            -$oldQty,
                            -$oldNetWeight
                        );
                    }
                ...
                if (!$stmt) {
                    throw new Exception('Failed to prepare old item delete.');
                }
                ...
                if (!$stmt->execute()) {
                    throw new Exception('Failed to delete old purchase items.');
                }
                ...
                --------------------------------------------- */
                if ($stockMovementExists && $stockMoveHasRefId) {
                    $sql = "DELETE FROM stock_movements WHERE ref_id = ?";

                    if ($stockMoveHasRefTable) {
                        $sql .= " AND ref_table = 'purchases'";
                    }
                ...
                foreach ($cleanItems as $row) {
                    ...
                    if ($productId > 0) {
                        updateProductStockQty($conn, $productId, $businessId, $prdHasBusinessId, $prdHasCurrentStockQty, $qty);
