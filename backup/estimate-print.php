...
$sale = null;
$stmt = $conn->prepare("SELECT s.*, pm.method_name FROM sales s LEFT JOIN payment_methods pm ON pm.id = s.payment_method_id WHERE s.id = ? AND s.business_id = ? AND s.bill_type = 'Estimate' LIMIT 1");
if ($stmt) {
...
class EstimatePDF extends FPDF
{
    ...
    public function Header(): void
    {
        ...
        $this->Cell(0, 7, $this->cancelled ? 'ESTIMATE - CANCELLED' : 'ESTIMATE', 'TB', 1, 'C');
    }
...
if ($pdf->cancelled) {
    ...
    $pdf->Cell(0, 8, 'CANCELLED ESTIMATE', 0, 1, 'C');
...
$pdf->Cell(96, 8, cleanPdfText('Estimate No: ' . ($sale['bill_no'] ?? '')), 1, 0, 'L');
$pdf->Cell(96, 8, cleanPdfText('Date: ' . $billDate . ' ' . $billTime), 1, 1, 'L');
$pdf->Cell(96, 8, cleanPdfText('Type: ' . ($sale['bill_type'] ?? '')), 1, 0, 'L');
...
$fileName = 'Estimate-' . preg_replace('/[^A-Za-z0-9_-]/', '-', (string)($sale['bill_no'] ?? $saleId)) . '.pdf';
