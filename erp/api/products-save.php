...
$sessionTaxType = (int)($_SESSION['non_gst_mode'] ?? 0) === 1 ? 'Non-GST' : 'GST';
if ($businessId <= 0)
...
if ($action === 'list') {
    ...
    if (!in_array($productType, ['', 'Gold', 'Silver', 'Diamond'], true)) $productType = '';
    ...
    if ($nonGstModeActive) {
        $taxType = trim((string)($_POST['tax_type'] ?? 'Non-GST'));
        if (!in_array($taxType, ['', 'GST', 'Non-GST'], true)) {
            $taxType = 'Non-GST';
        }
    } else {
        $taxType = 'GST';
    ...
    $taxTypeExpression = $hasTaxTypeColumn
        ? "CASE WHEN p.tax_type='Non-GST' THEN 'Non-GST' WHEN p.tax_type='GST' THEN 'GST' WHEN COALESCE(p.tax_percent,0)=0 THEN 'Non-GST' ELSE 'GST' END"
        : "CASE WHEN COALESCE(p.tax_percent,0)=0 THEN 'Non-GST' ELSE 'GST' END";

    if ($taxType !== '') {
        $where .= ' AND (' . $taxTypeExpression . ')=?';
        $params[] = $taxType;
        $types .= 's';
    }
...
COALESCE((SELECT SUM(ps.stock_value) FROM product_stock ps WHERE ps.product_id=p.id AND ps.business_id=p.business_id),0) AS stock_value FROM products p LEFT JOIN product_categories c ON c.id=p.category_id AND c.business_id=p.business_id LEFT JOIN metals m ON m.id=p.metal_id AND m.business_id=p.business_id LEFT JOIN units u ON u.id=p.unit_id AND u.business_id=p.business_id' . $where . ' ORDER BY p.id DESC LIMIT ? OFFSET ?';
    ...
    $statsSql = 'SELECT COUNT(DISTINCT p.id) AS total,COALESCE(SUM(p.is_active=1),0) AS active,COALESCE(SUM(p.is_active=0),0) AS inactive,COALESCE(SUM(p.track_stock=1),0) AS tracked FROM products p LEFT JOIN product_categories c ON c.id=p.category_id AND c.business_id=p.business_id LEFT JOIN metals m ON m.id=p.metal_id AND m.business_id=p.business_id' . $where;
...
if ($action === 'save') {
    ...
    $taxType = trim((string) ($_POST['tax_type'] ?? 'GST'));
    if (!in_array($taxType, ['GST', 'Non-GST'], true)) {
        $taxType = 'GST';
    }
    ...
    $data = [
        ...
        'product_type' => trim((string)($_POST['product_type'] ?? 'Gold')),
        'tax_type' => $taxType,
        ...
        'wastage_percent' => (float) ($_POST['wastage_percent'] ?? 0),
        'making_charge_type' => (string) ($_POST['making_charge_type'] ?? 'Per Gram'),
        'making_charge' => (float) ($_POST['making_charge'] ?? 0),
        'purchase_rate' => (float) ($_POST['purchase_rate'] ?? 0),
        'sale_rate' => (float) ($_POST['sale_rate'] ?? 0),
    ...
    if (!in_array($data['product_type'], ['Gold', 'Silver', 'Diamond'], true))
        respond(false, 'Please select a valid product type.');
    if (!tableHasColumn($conn, 'products', 'product_type'))
        respond(false, 'products.product_type column is missing. Run add-product-type-column.sql first.', [], 500);
    ...
    if ($isNew) {
        if ($hasTaxTypeColumn) {
            ...
            if (!$stmt) {
                respond(false, 'Unable to prepare product insert: ' . $conn->error, [], 500);
            }
        ...
        if ($hasTaxTypeColumn) {
            ...
            if (!$stmt) {
                respond(false, 'Unable to prepare product update: ' . $conn->error, [], 500);
            }
