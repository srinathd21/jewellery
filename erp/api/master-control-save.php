...
if (!hash_equals(
    (string) ($_SESSION['master_control_csrf'] ?? ''),
...
try {
    if ($action === 'save_number') {
        ...
        if (!in_array($key, ['invoice', 'estimate', 'purchase', 'pawn', 'chit'], true)) {
        ...
        $sample = strtr($template, [
            ...
            '{DIVIDER}' => $divider,
            '{CENTER}' => $sampleCenter,
            '{SEQ}' => str_pad((string) $start, $digits, '0', STR_PAD_LEFT),
            '{SUFFIX}' => $suffix
        ]);
        ...
        try {
            ...
            $stmt = $conn->prepare(
                'UPDATE document_number_settings
                 SET setting_name=?, prefix=?, center_format=?, suffix=?, divider=?,
                     sequence_digits=?, sequence_start=?, reset_frequency=?,
                     format_template=?, sample_output=?, is_active=1,
                     updated_at=CURRENT_TIMESTAMP
                 WHERE business_id=? AND branch_id=? AND document_key=?'
            );
            if (!$stmt) {
            ...
            if ($affectedRows === 0) {
                $check = $conn->prepare(
                    'SELECT id FROM document_number_settings
                     WHERE business_id=? AND branch_id=? AND document_key=? LIMIT 1'
                );
                if (!$check) {
                    throw new RuntimeException($conn->error);
                }
                ...
                if (!$exists) {
                    $stmt = $conn->prepare(
                        'INSERT INTO document_number_settings
                         (business_id,branch_id,document_key,setting_name,prefix,center_format,
                          suffix,divider,sequence_digits,sequence_start,reset_frequency,
                          format_template,sample_output,is_active)
                         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,1)'
                    );
                    if (!$stmt) {
                        throw new RuntimeException($conn->error);
            ...
            $cleanup = $conn->prepare(
                'DELETE d1 FROM document_number_settings d1
                 INNER JOIN document_number_settings d2
                    ON d1.business_id=d2.business_id
                   AND d1.branch_id=d2.branch_id
                   AND d1.document_key=d2.document_key
                   AND d1.id<d2.id
                 WHERE d1.business_id=? AND d1.branch_id=? AND d1.document_key=?'
            );
            if ($cleanup) {
    ...
    if ($action === 'save_metal') {
        ...
        $duplicate = $conn->prepare(
            'SELECT id FROM metals
             WHERE business_id=? AND (metal_code=? OR metal_name=?) AND id<>? LIMIT 1'
        );
        if (!$duplicate) {
            throw new RuntimeException($conn->error);
        }
        $duplicate->bind_param('issi', $businessId, $code, $name, $id);
        $duplicate->execute();
        if ($duplicate->get_result()->fetch_assoc()) {
            $duplicate->close();
            throw new RuntimeException('Metal code or metal name already exists.');
        }
        $duplicate->close();
        ...
        if ($id > 0) {
            $stmt = $conn->prepare(
                'UPDATE metals
                 SET metal_code=?, metal_name=?, default_purity=?, is_active=?
            ...
            $stmt = $conn->prepare(
                'INSERT INTO metals
                 (business_id,metal_code,metal_name,default_purity,is_active)
                 VALUES (?,?,?,?,?)'
            );
            if (!$stmt) {
                throw new RuntimeException($conn->error);
    ...
    if ($action === 'toggle_metal') {
        $id = (int) ($_POST['metal_id'] ?? 0);
        if ($id <= 0) {
            throw new RuntimeException('Invalid metal.');
        }

        $stmt = $conn->prepare(
            'UPDATE metals SET is_active=IF(is_active=1,0,1)
    ...
    if ($action === 'save_unit') {
        ...
        $code = strtoupper(trim((string) ($_POST['unit_code'] ?? '')));
        $name = trim((string) ($_POST['unit_name'] ?? ''));
        $decimalPlaces = max(0, min(6, (int) ($_POST['decimal_places'] ?? 3)));
        $active = (int) ($_POST['is_active'] ?? 1) === 1 ? 1 : 0;

        if ($code === '' || $name === '') {
            throw new RuntimeException('Unit code and unit name are required.');
        }

        if (!preg_match('/^[A-Z0-9_-]+$/', $code)) {
            throw new RuntimeException('Unit code may contain only letters, numbers, underscore and hyphen.');
        }

        $duplicate = $conn->prepare(
            'SELECT id FROM units
             WHERE business_id=? AND (unit_code=? OR unit_name=?) AND id<>? LIMIT 1'
        );
        if (!$duplicate) {
            throw new RuntimeException($conn->error);
        }
        ...
        if ($duplicate->get_result()->fetch_assoc()) {
            $duplicate->close();
            throw new RuntimeException('Unit code or unit name already exists.');
        }
        $duplicate->close();
        ...
        if ($id > 0) {
            $stmt = $conn->prepare(
                'UPDATE units
                 SET unit_code=?, unit_name=?, decimal_places=?, is_active=?
                 WHERE id=? AND business_id=? LIMIT 1'
            );
            if (!$stmt) {
                throw new RuntimeException($conn->error);
            }
            $stmt->bind_param('ssiiii', $code, $name, $decimalPlaces, $active, $id, $businessId);
        } else {
            $stmt = $conn->prepare(
                'INSERT INTO units
                 (business_id,unit_code,unit_name,decimal_places,is_active)
                 VALUES (?,?,?,?,?)'
            );
            if (!$stmt) {
                throw new RuntimeException($conn->error);
            }
            $stmt->bind_param('issii', $businessId, $code, $name, $decimalPlaces, $active);
        }
        ...
        $newId = $id ?: (int) $stmt->insert_id;
        $stmt->close();
        out(true, 'Unit saved successfully.', ['unit_id' => $newId]);
    }
    ...
    if ($action === 'toggle_unit') {
        ...
        $stmt = $conn->prepare(
            'UPDATE units SET is_active=IF(is_active=1,0,1)
             WHERE id=? AND business_id=?'
        );
        if (!$stmt) {
            throw new RuntimeException($conn->error);
        ...
        $stmt->close();
        out(true, 'Unit status changed successfully.');
    }
    ...
    if ($action === 'delete_unit') {
        ...
        $stmt = $conn->prepare('SELECT COUNT(*) c FROM products WHERE unit_id=? AND business_id=?');
        if (!$stmt) {
            throw new RuntimeException($conn->error);
        }
        $stmt->bind_param('ii', $id, $businessId);
        ...
        if ($used > 0) {
            throw new RuntimeException('This unit is used by products and cannot be deleted. Deactivate it instead.');
        }

        $stmt = $conn->prepare('DELETE FROM units WHERE id=? AND business_id=?');
        if (!$stmt) {
            throw new RuntimeException($conn->error);
        }
        $stmt->bind_param('ii', $id, $businessId);
        ...
        $stmt->close();
        out(true, 'Unit deleted successfully.');
    }
