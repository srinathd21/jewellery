...
if (!function_exists('hasChitCustomerPermission')) {
    function hasChitCustomerPermission(mysqli $conn, string $action): bool
    {
        $fieldMap = [
            'open' => 'can_open',
            'view' => 'can_view',
...
if (!hasChitCustomerPermission($conn, 'open')) {
    http_response_code(403);
    die('Access denied. You do not have permission to open chit customers.');
}

$canView = hasChitCustomerPermission($conn, 'view') || hasChitCustomerPermission($conn, 'open');
$canViewValue = hasChitCustomerPermission($conn, 'value') || $canView;
$canCreate = hasChitCustomerPermission($conn, 'create');
...
$pageTitle = 'Chit Customers';
$page_title = 'Chit Customers';
$currentPage = 'chit-customers';
...
<title><?php echo h($businessName); ?> - Chit Customers</title>
...
.filter-bar{display:flex;gap:8px;flex-wrap:wrap;padding:12px 13px;border-bottom:1px solid var(--border-color)}
...
.status-badge{display:inline-flex;border-radius:999px;padding:4px 8px;font-size:8px;font-weight:800}.status-active{background:#eaf8f0;color:#168449}.status-inactive{background:#edf1f5;color:#536170}
...
@media(max-width:991px){.responsive-table thead{display:none}.responsive-table tbody{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;padding:10px}.responsive-table tbody tr{display:grid;grid-template-columns:1fr 1fr;border:1px solid var(--border-color);border-radius:var(--radius);padding:12px}.responsive-table tbody td{display:flex;justify-content:space-between;gap:10px;padding:8px 0;border:0;border-bottom:1px dashed var(--border-color);text-align:right}.responsive-table tbody td::before{content:attr(data-label);font-size:8px;font-weight:700;text-transform:uppercase;color:var(--muted-color)}.responsive-table tbody td.main-column{grid-column:1/-1;display:block;text-align:left}.responsive-table tbody td.main-column::before{display:none}}
@media(max-width:767px){.page-heading,.panel-head{align-items:flex-start;flex-direction:column}.responsive-table tbody{grid-template-columns:1fr}.responsive-table tbody tr{grid-template-columns:1fr}.responsive-table tbody td{grid-column:1/-1}.filter-bar>*{width:100%}}
</style>
</head>
<body>
...
<div class="content-wrap">
    <div class="page-heading">
        <div>
            <h1 class="page-title">Chit Customers</h1>
            <div class="page-subtitle"><?php echo h($businessName); ?> · Customers enrolled in chit groups</div>
        </div>
        <?php if ($canCreate): ?>
            <a href="chit-customer-add.php" class="btn btn-theme">
    ...
    <?php if (!$canView): ?>
        <div class="panel"><div class="empty-state">You do not have permission to view chit customers.</div></div>
    <?php else: ?>
        <div class="panel">
            <div class="panel-head">
                <div>
                    <div class="panel-title">Customer List</div>
                    <div class="panel-subtitle">Customer details and chit collection summary.</div>
                </div>
            ...
            <div class="filter-bar">
                <input type="search" id="searchText" class="form-control" placeholder="Search code, name or mobile">
                <select id="statusFilter" class="form-select">
                    <option value="all">All Customers</option>
                    <option value="active">Active Chits</option>
                    <option value="inactive">No Active Chits</option>
                </select>
            </div>

            <div id="loadingState" class="empty-state">
                <i class="fa-solid fa-spinner fa-spin mb-2"></i>
                <div>Loading chit customers...</div>
            </div>
            ...
            <div id="emptyState" class="empty-state d-none">No chit customers found.</div>
        </div>
    <?php endif; ?>
...
(function(){
    ...
    function filteredRows(){
        ...
        return rows.filter(row=>{
            ...
            const matchesStatus=status==='all'
                ||(status==='active'&&Number(row.active_chits)>0)
                ||(status==='inactive'&&Number(row.active_chits)===0);
    ...
    function render(){
        ...
        body.innerHTML=data.map((row,index)=>`<tr>
            <td data-label="#">${index+1}</td>
            <td data-label="Code"><strong>${escapeHtml(row.customer_code)}</strong></td>
            <td class="main-column" data-label="Customer">
                <div class="main-text">${escapeHtml(row.customer_name)}</div>
                <div class="subtext">${escapeHtml(row.address_line1||'')}</div>
            </td>
            <td data-label="Mobile">${row.mobile?`<a href="tel:${escapeHtml(row.mobile)}">${escapeHtml(row.mobile)}</a>`:'—'}</td>
            <td data-label="Nominee">${escapeHtml(row.nominee_name||'—')}</td>
            <td data-label="Active Chits">${Number(row.active_chits)}</td>
            <td data-label="Paid">${canViewValue?'₹'+money(row.total_paid_amount):'••••'}</td>
            <td data-label="Due">${canViewValue?'₹'+money(row.total_due_amount):'••••'}</td>
            <td data-label="Status">
                <span class="status-badge ${Number(row.active_chits)>0?'status-active':'status-inactive'}">
                    ${Number(row.active_chits)>0?'Active':'Inactive'}
                </span>
            </td>
        </tr>`).join('');
    }
    ...
    async function loadCustomers(){
        ...
        try{
            const response=await fetch('api/chit-customers.php?action=list',{credentials:'same-origin'});
            const result=await response.json().catch(()=>({success:false,message:'Invalid JSON response from API.'}));

            if(!response.ok||!result.success){
                throw new Error(result.message||'Unable to load chit customers.');
            }
