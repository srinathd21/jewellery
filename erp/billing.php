...
function billingColumnExists(mysqli $conn, string $table, string $column): bool
{
...
$pageNonGstModeActive = $nonGstModeActive;
if ($editSaleId > 0) {
    $modeStmt = $conn->prepare("SELECT tax_type FROM sales WHERE id=? AND business_id=? AND branch_id=? LIMIT 1");
    if ($modeStmt) {
        $modeStmt->bind_param('iii', $editSaleId, $businessId, $branchId);
        $modeStmt->execute();
        $modeRow = $modeStmt->get_result()->fetch_assoc();
        $modeStmt->close();
        if ($modeRow) {
            $pageNonGstModeActive = strtolower(trim((string)($modeRow['tax_type'] ?? 'GST'))) === 'non-gst';
        }
    }
}
...
$productTaxCondition = $productHasTaxType
    ? ($pageNonGstModeActive ? " AND p.tax_type='Non-GST'" : " AND p.tax_type='GST'")
    : ($pageNonGstModeActive ? " AND COALESCE(p.tax_percent,0)=0" : " AND COALESCE(p.tax_percent,0)>0");

$productSql = "SELECT p.id,p.product_code,p.barcode,p.product_name,p.product_type,
                      {$productTaxSelect} AS tax_type,
                      ...
                      m.metal_name,
                      COALESCE(mr.rate_per_gram,p.sale_rate,0) AS live_metal_rate,
                      mr.effective_from AS live_rate_effective_from,
                      mr.branch_id AS live_rate_branch_id
               FROM products p
               ...
               LEFT JOIN metal_rates mr ON mr.id=(
                    SELECT mr2.id
                    FROM metal_rates mr2
                    WHERE mr2.business_id=p.business_id
                      AND mr2.metal_id=p.metal_id
                      AND mr2.is_current=1
                      AND (mr2.branch_id=? OR mr2.branch_id IS NULL)
                    ORDER BY (mr2.branch_id=?) DESC,mr2.effective_from DESC,mr2.id DESC
                    LIMIT 1
               )
               WHERE p.business_id=?
                 AND p.is_active=1
...
$metalSql = "SELECT m.id,m.metal_name,
             ...
             FROM metals m
             WHERE m.business_id=?";
if (billingColumnExists($conn, 'metals', 'is_active')) {
    $metalSql .= " AND m.is_active=1";
}
$metalSql .= " ORDER BY m.metal_name";
$metalStmt = $conn->prepare($metalSql);
if ($metalStmt) {
    $metalStmt->bind_param('iiii', $businessId, $branchId, $branchId, $businessId);
    $metalStmt->execute();
    $metalResult = $metalStmt->get_result();
    while ($metalRow = $metalResult->fetch_assoc()) {
        $exchangeMetals[] = $metalRow;
    }
    $metalStmt->close();
}
...
{
    $documentKey = $documentType === 'Non GST Invoice' ? 'non_gst_invoice' : strtolower($documentType);
    $stmt = $conn->prepare(
...
if ($editSaleId > 0) {
    ...
    if (!$editPayments && (float)($saleEdit['paid_amount'] ?? 0) > 0 && billingColumnExists($conn, 'customer_payments', 'sale_id')) {
        $s = $conn->prepare("SELECT cps.payment_method_id,cps.amount,cps.reference_no
                             FROM customer_payments cp
                             INNER JOIN customer_payment_splits cps ON cps.payment_id=cp.id
                             WHERE cp.sale_id=? AND cp.business_id=? ORDER BY cp.id,cps.id");
        if ($s) {
            $s->bind_param('ii', $editSaleId, $businessId);
            $s->execute();
            $r = $s->get_result();
...
$defaultDocumentType = $pageNonGstModeActive ? 'Non GST Invoice' : 'Invoice';
...
<body>
                        ...
                        <div class="bill-body">
                            <div class="bill-detail-grid">
                                ...
                                <div class="c3"><label class="field-label">Bill Type</label><select name="bill_type"
                                        ...
                                        <?php if ($pageNonGstModeActive): ?>
                                            <option value="Non GST" selected>Non GST</option>
                                        <?php else: ?>
                                            <option value="Retail">Retail</option>
                                            <option value="GST">GST</option>
                                            <?php if ($editSaleId <= 0): ?><option value="Estimate">Estimate</option><?php endif; ?>
    ...
    <script>
        (function () {
            ...
            const nonGstModeActive = <?= $pageNonGstModeActive ? 'true' : 'false' ?>;
            ...
            async function refreshDocumentNumber() {
                ...
                const isNonGst = nonGstModeActive;
                const type = isEstimate ? 'Estimate' : (isNonGst ? 'Non GST Invoice' : 'Invoice');

                if (documentMode) documentMode.value = type;
                if (documentNumberLabel) {
                    documentNumberLabel.textContent = isEstimate
                        ? 'Estimate No'
                        : (isNonGst ? 'Non GST Invoice No' : 'Bill No');
                }
                ...
                try {
                    ...
                    if (requestId !== documentNumberRequestId) return;
                    documentNumberPreview.value = type === 'Estimate' ? 'EST-NOT-CONFIGURED' : (type === 'Non GST Invoice' ? 'NGST-NOT-CONFIGURED' : 'INV-NOT-CONFIGURED');
            ...
            function startBillingPage() {
                ...
                function productOptions() {
                    return '<option value="">Select product</option>' + products.map(p =>
                        `<option value="${Number(p.id)}">${esc(p.product_name + ' - ' + p.product_code + ' [' + (p.tax_type || (nonGstModeActive ? 'Non-GST' : 'GST')) + '] (Stock: ' + Number(p.stock_qty).toFixed(3) + ')')}</option>`
                    ).join('');
                }
                ...
                function loadEditData() {
                    ...
                    if (billTypeSelect) billTypeSelect.value = nonGstModeActive ? 'Non GST' : String(sale.bill_type || 'Retail');
                ...
                function calc() {
                    ...
                    let split = 0;
                    let received = 0;
                    pays.querySelectorAll('.payment-row').forEach(row => {
                        const methodId = row.querySelector('.payment-method')?.value || '';
                        const amount = Number(row.querySelector('.pay-amount')?.value || 0) || 0;
                        const method = payments.find(x => String(x.id) === String(methodId));
                        const methodName = String(method?.method_name || '').toLowerCase();
                        const methodType = String(method?.method_type || '').toLowerCase();
                        const isCredit = methodType === 'credit' || methodName.includes('credit') || methodName.includes('due') || methodName.includes('pay later') || methodName.includes('paylater');
                        split += amount;
                        if (!isCredit) received += amount;
                    });
                    const paid = Math.min(received, grand);
                    ...
                    document.getElementById('paidAmount').value = money(received);
                ...
                async function loadCustomerChits(options = {}) {
                    const preserveClaims = !!(options && options.preserveClaims);
                    customerChits = [];
                    if (!preserveClaims) appliedClaims = [];
                    calc();
                    ...
                    try {
                        ...
                        customerChits = (data.chits || []).map(chit => {
                            const currentSaleClaim = preserveClaims
                                ? appliedClaims.filter(claim => Number(claim.chit_member_id) === Number(chit.chit_member_id))
                                    .reduce((sum, claim) => sum + Number(claim.claim_grams || 0), 0)
                                : 0;
                            return {
                                ...chit,
                                saved_grams: Math.max(0, Number(chit.saved_grams || 0)),
                                claimed_grams: Math.max(0, Number(chit.claimed_grams || 0) - currentSaleClaim),
                                available_grams: Math.max(0, Number(chit.available_grams || 0) + currentSaleClaim)
                            };
                        });
                ...
                function exchangeMetalOptions(selected = '') {
                    return '<option value="">Select metal type</option>' + exchangeMetals.map(metal =>
                        `<option value="${Number(metal.id)}" ${String(metal.id) === String(selected) ? 'selected' : ''}>${esc(metal.metal_name)}</option>`
                    ).join('');
                }

                function currentExchangeMetal(metalId) {
                    return exchangeMetals.find(metal => String(metal.id) === String(metalId));
                }
                function renderExchangeItems() {
                    const wrap = document.getElementById('exchangeItems');
                    wrap.innerHTML = exchangeItems.map((x, i) => `
                        <div class="adjustment-card" data-index="${i}" data-exchange-id="${Number(x.id || 0)}">
                            <div class="adjustment-grid">
                                <div class="wide">
                                    <label class="field-label">Old Item Name</label>
                                    <input class="form-control ex-name" value="${esc(x.item_name || '')}">
                                </div>
                                <div>
                                    <label class="field-label">Metal Type</label>
                                    <select class="form-select ex-metal">${exchangeMetalOptions(x.metal_id || '')}</select>
                                </div>
                                <div>
                                    <label class="field-label">Gross Gram</label>
                                    <input type="number" min="0" step="0.001" class="form-control ex-gross" value="${Number(x.gross_weight || 0)}">
                                </div>
                                <div>
                                    <label class="field-label">Wastage %</label>
                                    <input type="number" min="0" max="100" step="0.001" class="form-control ex-waste" value="${Number(x.wastage_percent || 0)}">
                                </div>
                                <div>
                                    <label class="field-label">Eligible Gram</label>
                                    <input class="form-control ex-net" readonly value="${Number(x.eligible_weight || 0).toFixed(3)}">
                                </div>
                                <div>
                                    <label class="field-label">Current Rate</label>
                                    <input type="number" min="0" step="0.01" class="form-control ex-rate" value="${Number(x.rate_per_gram || 0)}" readonly>
                                </div>
                                <div>
                                    <label class="field-label">Value</label>
                                    <input type="number" min="0" step="0.01" class="form-control ex-value" value="${money(x.exchange_value || 0)}">
                                </div>
                                <button type="button" class="btn btn-outline-danger remove-exchange"><i class="fa-solid fa-trash"></i></button>
                            </div>
                        </div>`).join('');
                    document.getElementById('exchangeTotal').textContent = money(exchangeTotalValue());
                    calc();
                }

                function syncExchange(event = null) {
                    const editedValueInput = event && event.target && event.target.classList.contains('ex-value')
                        ? event.target
                        : null;
                    const nextItems = [];

                    document.querySelectorAll('#exchangeItems .adjustment-card').forEach(card => {
                        const metalSelect = card.querySelector('.ex-metal');
                        const metal = currentExchangeMetal(metalSelect.value);
                        const gross = Number(card.querySelector('.ex-gross').value) || 0;
                        const waste = Number(card.querySelector('.ex-waste').value) || 0;
                        const rateInput = card.querySelector('.ex-rate');
                        const metalChanged = event && event.target === metalSelect;
                        if (metalChanged) {
                            rateInput.value = money(metal?.current_rate || 0);
                        }
                        const rate = Number(rateInput.value) || 0;
                        const eligible = Math.max(0, gross * (1 - waste / 100));
                        const calculatedValue = eligible * rate;
                        const valueInput = card.querySelector('.ex-value');
                        let value;
                        ...
                        } else if (changedCalculationField) {
                            value = calculatedValue;
                            valueInput.value = money(value);
                        } else {
                            value = Math.max(0, Number(valueInput.value) || calculatedValue);
                        }

                        card.querySelector('.ex-net').value = eligible.toFixed(3);
                        nextItems.push({
                            id: Number(card.dataset.exchangeId || 0),
                            item_name: card.querySelector('.ex-name').value.trim(),
                            metal_id: Number(metalSelect.value) || 0,
                    ...
                    exchangeItems = nextItems;
                    document.getElementById('exchangeTotal').textContent = money(exchangeTotalValue());
                    calc();
                }
                document.getElementById('addExchange').addEventListener('click', () => { const firstMetal = exchangeMetals[0] || null; exchangeItems.push({ id: 0, item_name: 'Old Gold', metal_id: Number(firstMetal?.id || 0), metal_name: firstMetal?.metal_name || '', gross_weight: 0, wastage_percent: 0, eligible_weight: 0, rate_per_gram: Number(firstMetal?.current_rate || 0), exchange_value: 0 }); renderExchangeItems() });
                document.getElementById('exchangeItems').addEventListener('input', syncExchange); document.getElementById('exchangeItems').addEventListener('change', syncExchange); document.getElementById('exchangeItems').addEventListener('click', e => { const b = e.target.closest('.remove-exchange'); if (!b) return; exchangeItems.splice(Number(b.closest('.adjustment-card').dataset.index), 1); renderExchangeItems() });

                async function switchBillingTaxMode() {
                    try {
                        const fd = new FormData();
                        fd.append('csrf_token', csrfToken);
                        const response = await fetch('api/toggle-non-gst-mode.php', {
                            method: 'POST',
                            body: fd,
                        ...
                        if (!response.ok || !result.success) {
                            throw new Error(result.message || 'Unable to switch billing mode.');
                        }
                        toast('success', result.non_gst_mode ? 'Non-GST mode enabled.' : 'GST mode enabled.');
                ...
                document.getElementById('gstShortcutToggle')?.addEventListener('click', switchBillingTaxMode);

                window.addEventListener('keydown', event => {
                    const isShortcut = event.ctrlKey && event.shiftKey && String(event.key).toLowerCase() === 'g';
                    if (!isShortcut) return;
                    event.preventDefault();
                    event.stopPropagation();
                    event.stopImmediatePropagation();
                    switchBillingTaxMode();
                }, true);
                ...
                ensureSelect2().then(ready => {
                    ...
                    if (editMode && window.jQuery) {
                        ...
                        if (billTypeSelect) window.jQuery(billTypeSelect).val(nonGstModeActive ? 'Non GST' : String(editData.sale.bill_type || '')).trigger('change.select2');
