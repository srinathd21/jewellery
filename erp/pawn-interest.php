...
<body>
    ...
    <script>
        (function () {
            ...
            async function calculatePayment() { const id = $('pawnSelect').value; if (!id) return; try { paymentQuote = await req({ action: 'payment_quote', pawn_id: id, as_of_date: $('asOfDate').value }); $('paymentInterestDue').value = Number(paymentQuote.interest_outstanding || 0).toFixed(2); $('sumPrincipalBalance').textContent = money(paymentQuote.principal_balance); $('sumPaymentInterest').textContent = money(paymentQuote.interest_outstanding); updatePaymentSummary(); updateClosureState() } catch (e) { toast(false, e.message) } }
            function updatePaymentSummary() { const p = pawn(); let amt = Math.max(0, Number($('principalPayment').value || 0)); const bal = Number(paymentQuote ? paymentQuote.principal_balance : (p ? p.balance_principal : 0)); if (amt > bal) { amt = bal; $('principalPayment').value = bal.toFixed(2) } const after = Math.max(0, bal - amt); $('paymentBalanceAfter').value = after.toFixed(2); $('sumPrincipalPaying').textContent = money(amt); $('sumPaymentTotal').textContent = money(amt) }
            function updateClosureState() { const on = $('isClosure').checked; $('closureBox').classList.toggle('show', on); $('releasedTo').required = on; if (!on) { $('paymentSaveBtn').disabled = false; return } const q = paymentQuote || {}; $('principalPayment').value = Number(q.principal_balance || 0).toFixed(2); updatePaymentSummary(); let msg = '', block = false; if (Number(q.interest_outstanding || 0) > 0.009) { msg = 'Collect all pending interest first: ₹' + money(q.interest_outstanding) + '.'; block = true } else if (q.bank_release_blocked) { msg = 'Closure blocked: this pawn gold is still actively pledged with a bank.'; block = true } else { msg = 'Ready for full closure. A pending gold handover/release record will be created.' } $('closureWarning').textContent = msg; $('closureWarning').className = 'small mt-2 ' + (block ? 'text-danger' : 'text-success'); $('paymentSaveBtn').disabled = block }
