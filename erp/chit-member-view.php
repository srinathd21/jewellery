...
function whatsappNumber($mobile): string
{
    ...
    if (strlen($number) === 10) {
        $number = '91' . $number;
    } elseif (strlen($number) === 11 && str_starts_with($number, '0')) {
...
<head>
    ...
    <style>
        :root {
            --p:
                <?= h($theme['primary_color']) ?>
            ;
            --pd:
                <?= h($theme['primary_dark_color']) ?>
            ;
            --bg:
                <?= h($theme['page_background']) ?>
            ;
            --card:
                <?= h($theme['card_background']) ?>
            ;
            --text:
                <?= h($theme['text_color']) ?>
            ;
            --muted:
                <?= h($theme['muted_text_color']) ?>
            ;
            --line:
                <?= h($theme['border_color']) ?>
            ;
            --r:
        ...
        body.dark-mode .btn-invoice,
        body[data-theme=dark] .btn-invoice,
        html.dark-mode body .btn-invoice,
        html[data-theme=dark] body .btn-invoice {
            background: #22303a;
            color: #eef4f7;
            border-color: #3a4b57;
        }
                    ...
                    <table class="table">
                        ...
                        <tbody><?php foreach ($collections as $c): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        $collectionId = (int) ($c[$collectionPk] ?? 0);
                                        $receiptNo = (string) ($c['receipt_no'] ?? '');
                                        $invoiceRelativeUrl = 'chit-invoice.php?collection_id=' . $collectionId
                                            . '&receipt=' . rawurlencode($receiptNo);
                                        $invoiceUrl = absoluteUrl($invoiceRelativeUrl);

                                        $waNumber = whatsappNumber($m['mobile'] ?? '');
                                        $waMessage = "Dear " . (string) $m['customer_name'] . ",\n\n"
                                            . "Thank you for your chit payment.\n"
                                            . "Receipt: " . $receiptNo . "\n"
                                            . "Installment: " . (string) ($c['installment_no'] ?? '') . "\n"
                                            . "Amount Paid: ₹" . number_format((float) ($c['net_amount'] ?? $c['paid_amount'] ?? 0), 2) . "\n"
                                            . "Date: " . (!empty($c['collection_date']) ? date('d-m-Y', strtotime($c['collection_date'])) : '') . "\n\n"
                                            . "View / download your chit invoice here:\n" . $invoiceUrl . "\n\n"
                                            . "Thank you,\n" . $businessName;
                                ...
                                <tr>
                                    <td colspan="<?= (($m['chit_type'] ?? '') === 'Gold') ? 12 : 10 ?>"
                                        class="text-center text-muted py-4">No payments collected.</td>
                                </tr><?php endif; ?>
                        </tbody>
                    </table>
                </div>
