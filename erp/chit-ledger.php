        ...
        <?php else: ?>
            <div class="table-responsive">
                <table class="table responsive-table">
                    <thead><tr><th>#</th><th>Receipt</th><th>Group / Member</th><th>Installment</th><th>Collected</th><th>Due</th><th>Paid</th><th>Discount</th><th>Penalty</th><th>Net Received</th><th>Status</th><th>Reference</th><th>Actions</th></tr></thead>
                    <tbody>
                    <?php foreach ($ledgerRows as $index => $row): ?>
                        ...
                        <tr>
                            ...
                            <td data-label="Actions">
                                <?php
                                $invoiceRelativeUrl = 'chit-invoice.php?collection_id=' . (int)$row['collection_id']
                                    . '&receipt=' . rawurlencode((string)$row['receipt_no']);
                                $invoiceUrl = absoluteUrl($invoiceRelativeUrl);

                                $waNumber = whatsappNumber($row['mobile'] ?? '');
                                $waMessage = "Dear " . (string)$row['customer_name'] . ",\n\n"
                                    . "Thank you for your chit payment.\n"
                                    . "Receipt: " . (string)$row['receipt_no'] . "\n"
                                    . "Group: " . (string)$row['group_no'] . " - " . (string)$row['group_name'] . "\n"
                                    . "Ticket: " . (string)$row['ticket_no'] . "\n"
                                    . "Installment: " . (string)$row['installment_no'] . "\n"
                                    . "Paid: " . $currency . number_format((float)$row['paid_amount'], 2) . "\n"
                                    . "Net Received: " . $currency . number_format((float)$row['net_amount'], 2) . "\n"
                                    . "Date: " . date('d-m-Y', strtotime($row['collection_date'])) . "\n\n"
                                    . "View your chit invoice:\n" . $invoiceUrl . "\n\n"
                                    . "Thank you,\n" . $businessName;
                                ...
                                <div class="row-actions">
                                    <a class="btn-row-action btn-row-invoice"
                                       href="<?php echo h($invoiceRelativeUrl); ?>"
                                       target="_blank" rel="noopener">
                                        <i class="fa-solid fa-file-invoice"></i> Invoice
                                    </a>
                                    <?php if ($waUrl !== ''): ?>
                                        <a class="btn-row-action btn-row-whatsapp"
                                           href="<?php echo h($waUrl); ?>"
                                           target="_blank" rel="noopener">
                                            <i class="fa-brands fa-whatsapp"></i> WhatsApp
                                        </a>
                                    <?php else: ?>
                                        <span class="sub-text">No mobile</span>
                                    <?php endif; ?>
                                </div>
                            </td>
                        </tr>
