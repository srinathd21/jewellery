...
if ($nonGstModeActive && $requestedTaxType === '') {
    $taxTypeFilter = 'Non-GST';
}

if ($taxTypeFilter === 'GST') {
    $where .= " AND COALESCE(p.tax_type, CASE WHEN COALESCE(p.tax_percent,0)>0 THEN 'GST' ELSE 'Non-GST' END)='GST'";
} elseif ($taxTypeFilter === 'Non-GST') {
    $where .= " AND COALESCE(p.tax_type, CASE WHEN COALESCE(p.tax_percent,0)>0 THEN 'GST' ELSE 'Non-GST' END)='Non-GST'";
}
if ($search !== '') {
...
$summarySql = 'SELECT COUNT(DISTINCT p.id) total_products,
COALESCE(SUM(CASE WHEN COALESCE(ps.quantity,0)>0 THEN 1 ELSE 0 END),0) in_stock,
...
<head>
    ...
    <style>
        ...
        .filter-grid {
            display: grid;
            grid-template-columns: minmax(240px, 1.8fr) repeat(<?= $nonGstModeActive ? 6 : 5 ?>, minmax(120px, 1fr));
            gap: 8px;
            align-items: end
        }
                                        ...
                                        <td>
                                            <div class="d-flex gap-1"><a class="action-btn" title="Adjust Stock"
                                                    ...
                                                    href="stock-movements.php?product_id=<?= (int) $p['id'] ?>&branch_id=<?= $branchId ?>"><i
                                                        class="fa-solid fa-right-left"></i></a></div>
                                        </td>
                                    </tr>
                                <?php endforeach; else: ?>
                                <tr>
