...
$initialProductType = (string)($_GET['product_type'] ?? '');
if (!in_array($initialProductType, ['', 'Gold', 'Silver', 'Diamond'], true)) $initialProductType = '';
...
$requestedTaxType = (string)($_GET['tax_type'] ?? '');
$initialTaxType = $nonGstModeActive
    ? (in_array($requestedTaxType, ['GST', 'Non-GST'], true) ? $requestedTaxType : 'Non-GST')
    : 'GST';
...
<head>
    ...
    <style>
        ...
        .products-filter-grid.has-tax-filter {
            grid-template-columns: minmax(210px, 1.65fr) minmax(145px, .9fr) minmax(135px, .82fr) minmax(145px, .9fr) 100px auto auto;
        }
        ...
        @media (max-width: 1200px) and (min-width: 992px) {
            ...
            .products-filter-grid.has-tax-filter {
                grid-template-columns: minmax(180px, 1.42fr) minmax(125px, .82fr) minmax(120px, .74fr) minmax(130px, .8fr) 92px auto auto;
            }
    ...
    <div class="content-wrap">
        ...
        <?php else: ?>
            ...
            <div class="products-toolbar">
                <form id="productsFilterForm" class="products-filter-grid<?php echo $nonGstModeActive ? ' has-tax-filter' : ''; ?>" autocomplete="off">
                    ...
                    <div class="filter-field has-icon">
                        ...
                        <select class="form-select" id="productTypeFilter" name="product_type">
                            <option value="">All product types</option>
                            <option value="Gold" <?php echo $initialProductType === 'Gold' ? 'selected' : ''; ?>>Gold</option>
                            <option value="Silver" <?php echo $initialProductType === 'Silver' ? 'selected' : ''; ?>>Silver</option>
                            <option value="Diamond" <?php echo $initialProductType === 'Diamond' ? 'selected' : ''; ?>>Diamond</option>
                        </select>
                    </div>
                    ...
                    <?php if ($nonGstModeActive): ?>
                        <div class="filter-field has-icon">
                            ...
                            <select class="form-select" id="taxTypeFilter" name="tax_type">
                                <option value="" <?php echo $initialTaxType === '' ? 'selected' : ''; ?>>All Products</option>
                                <option value="GST" <?php echo $initialTaxType === 'GST' ? 'selected' : ''; ?>>GST Products</option>
                                <option value="Non-GST" <?php echo $initialTaxType === 'Non-GST' ? 'selected' : ''; ?>>Non-GST Products</option>
                            </select>
                        </div>
                    <?php endif; ?>
...
(function () {
    ...
    const taxTypeFilter = document.getElementById('taxTypeFilter');
            ...
            '<div class="view-product-layout">' +
                ...
                '<div>' +
                    '<div class="view-section"><div class="view-section-title">Basic Information</div><div class="view-detail-grid">' +
                        ...
                        detailItem('Product Type', product.product_type || 'Gold') +
                        detailItem('GST Type', product.tax_type || 'GST') +
                        detailItem('HSN Code', (product.tax_type === 'Non-GST' ? '—' : product.hsn_code)) +
                    '</div></div>' +
                    ...
                    '<div class="view-section"><div class="view-section-title">Charges, Rates & Tax</div><div class="view-detail-grid">' +
                        ...
                        detailItem('Tax %', product.tax_type === 'Non-GST' ? '—' : number(product.tax_percent, 3)) +
                    '</div></div>' +
    ...
    function collectState(page = currentPage) {
        return {
            ...
            tax_type: taxTypeFilter ? taxTypeFilter.value : 'GST',
    ...
    function updateUrl(state, replace = false) {
        ...
        ['search', 'category_id', 'product_type', 'status', 'tax_type', 'per_page', 'page'].forEach(key => url.searchParams.delete(key));

        if (state.search) url.searchParams.set('search', state.search);
        if (Number(state.category_id) > 0) url.searchParams.set('category_id', state.category_id);
        if (state.product_type) url.searchParams.set('product_type', state.product_type);
        if (state.status) url.searchParams.set('status', state.status);
        if (taxTypeFilter && state.tax_type) url.searchParams.set('tax_type', state.tax_type);
    ...
    function productRow(product) {
        ...
        return '<tr>' +
            ...
            '<td data-label="Product Type"><span class="badge-soft">' + escapeHtml(product.product_type || 'Gold') + '</span></td>' +
    ...
    [categoryFilter, productTypeFilter, statusFilter, taxTypeFilter, perPageFilter].filter(Boolean).forEach(element => {
    ...
    resetFilters.addEventListener('click', () => {
        ...
        statusFilter.value = '';
        if (taxTypeFilter) taxTypeFilter.value = 'Non-GST';
    ...
    window.addEventListener('popstate', () => {
        ...
        productTypeFilter.value = params.get('product_type') || '';
        statusFilter.value = params.get('status') || '';
        if (taxTypeFilter) {
            const requestedTaxType = params.get('tax_type');
            taxTypeFilter.value = ['GST', 'Non-GST'].includes(requestedTaxType) ? requestedTaxType : 'Non-GST';
        }
