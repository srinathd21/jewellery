...
if ($businessId <= 0) { die('Business session not found. Please login again.'); }
...
$postShopName          = $oldInput['shop_name'] ?? extractNoteValue($currentNotes, 'Shop/Business Name');
$postCustomerName      = $oldInput['customer_name'] ?? ($customer['customer_name'] ?? '');
$postCustomerContact   = $oldInput['customer_contact'] ?? ($customer['mobile'] ?? '');
$postAlternateContact  = $oldInput['alternate_contact'] ?? ($customer['alternate_mobile'] ?? '');
$postShopLocation      = $oldInput['shop_location'] ?? ($customer['address_line1'] ?? '');
$postGstNumber         = $oldInput['gst_number'] ?? ($customer['gstin'] ?? '');
$postEmail             = $oldInput['email'] ?? ($customer['email'] ?? '');
$postCustomerCategory  = $oldInput['customer_category'] ?? strtolower(extractNoteValue($currentNotes, 'Customer Category', 'retail'));
$postAssignedLinemanId = (int)($oldInput['assigned_lineman_id'] ?? extractNoteValue($currentNotes, 'Assigned Lineman ID', '0'));
$postAssignedArea      = $oldInput['assigned_area'] ?? extractNoteValue($currentNotes, 'Assigned Area');
$postPaymentTerms      = $oldInput['payment_terms'] ?? extractNoteValue($currentNotes, 'Payment Terms', 'cash');
$postCreditLimit       = $oldInput['credit_limit'] ?? str_replace(['₹', ','], '', extractNoteValue($currentNotes, 'Credit Limit', '0'));
$postNotes             = $oldInput['notes'] ?? extractNoteValue($currentNotes, 'Notes');
$postZoneId            = (int)($oldInput['zone_id'] ?? extractNoteValue($currentNotes, 'Zone ID', '0'));
...
$theme = [
    ...
    'muted_text_color'=>'#7d8794','border_color'=>'#e8e8e8','border_radius_px'=>12,'sidebar_width_px'=>230,
];
if (tableExists($conn, 'business_theme_settings')) {
    ...
    if ($themeStmt) {
        ...
        foreach ($theme as $key => $defaultValue) {
            if (isset($themeRow[$key]) && $themeRow[$key] !== '') $theme[$key] = $themeRow[$key];
        }
    }
}
...
<head>
    ...
    <title><?php echo h($businessName); ?> - Edit Customer</title>
                    ...
                    <div class="col-12">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <div>
                                ...
                                <div class="text-muted">Update customer account details</div>
                            </div>
                            <div>
