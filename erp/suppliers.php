...
function supplierTableExists(mysqli $conn, string $table): bool
{
...
function supplierColumnExists(mysqli $conn, string $table, string $column): bool
{
...
$overallPendingAmount = 0.00;

if (supplierTableExists($conn, 'purchases')) {
    $pendingSql = "SELECT COALESCE(SUM(balance_amount),0) pending_amount
                   FROM purchases
                   WHERE business_id=?";
    $pendingTypes = 'i';
    $pendingParams = [$businessId];

    if ($branchId > 0 && supplierColumnExists($conn, 'purchases', 'branch_id')) {
        $pendingSql .= " AND branch_id=?";
        $pendingTypes .= 'i';
        $pendingParams[] = $branchId;
    }

    if (supplierColumnExists($conn, 'purchases', 'workflow_status')) {
        $pendingSql .= " AND COALESCE(workflow_status,'Posted')<>'Cancelled'";
    }

    $pendingStmt = $conn->prepare($pendingSql);
    if ($pendingStmt) {
        if ($pendingTypes === 'ii') {
            $pendingStmt->bind_param($pendingTypes, $pendingParams[0], $pendingParams[1]);
        } else {
            $pendingStmt->bind_param($pendingTypes, $pendingParams[0]);
        }
        $pendingStmt->execute();
        $overallPendingAmount = (float)($pendingStmt->get_result()->fetch_assoc()['pending_amount'] ?? 0);
        $pendingStmt->close();
    }
}
...
if (supplierTableExists($conn, 'purchases')) {
    $supplierDueSql = "SELECT supplier_id, COALESCE(SUM(balance_amount),0) due_amount
                       FROM purchases
                       WHERE business_id=?";
    $supplierDueTypes = 'i';
    $supplierDueParams = [$businessId];

    if ($branchId > 0 && supplierColumnExists($conn, 'purchases', 'branch_id')) {
        $supplierDueSql .= " AND branch_id=?";
        $supplierDueTypes .= 'i';
        $supplierDueParams[] = $branchId;
    }

    if (supplierColumnExists($conn, 'purchases', 'workflow_status')) {
        $supplierDueSql .= " AND COALESCE(workflow_status,'Posted')<>'Cancelled'";
    }

    $supplierDueSql .= " GROUP BY supplier_id";

    $supplierDueStmt = $conn->prepare($supplierDueSql);
    if ($supplierDueStmt) {
        if ($supplierDueTypes === 'ii') {
            $supplierDueStmt->bind_param(
                $supplierDueTypes,
                $supplierDueParams[0],
                $supplierDueParams[1]
            );
        ...
        $supplierDueStmt->execute();
        $supplierDueResult = $supplierDueStmt->get_result();

        while ($supplierDueRow = $supplierDueResult->fetch_assoc()) {
            $supplierDueMap[(int)$supplierDueRow['supplier_id']] =
                (float)$supplierDueRow['due_amount'];
        }

        $supplierDueStmt->close();
    }
}
    ...
    <div class="content-wrap">
        <div class="page-card mb-3">
            <div class="page-head">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    <a href="supplier-ledger.php" class="btn-soft text-decoration-none">
                        <i class="fa-solid fa-book-open me-1"></i>Supplier Ledger
                    </a>
        ...
        <div class="stat-grid">
            ...
            <div class="stat-card">
                ...
                <div>
                    <div class="stat-label">Overall Pending Pay</div>
                    <div class="stat-value">₹<?= number_format($overallPendingAmount, 2) ?></div>
                </div>
            </div>
            <div class="stat-card">
                ...
                <table class="table supplier-table">
                    ...
                    <tr>
                        ...
                        <?php if ($canValue): ?>
                            ...
                            <th class="text-end">Due Pending</th>
...
(() => {
    ...
    const supplierDueMap = <?= json_encode($supplierDueMap, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
            ...
            body.innerHTML = result.suppliers.map(row => `
                <tr>
                    ...
                    ${canValue ? `
                        ...
                        <td data-label="Due Pending" class="text-end">
                            ${Number(supplierDueMap[row.id] || 0) > 0
                                ? `<span class="due-amount">₹${money(supplierDueMap[row.id])}</span>`
                                : `<span class="due-cleared">₹0.00</span>`
                            }
                        </td>
                    ...
                    <td data-label="Actions" class="text-end">
                        <div class="action-group">
                            <a class="action-btn" href="supplier-ledger.php?supplier_id=${row.id}" title="Ledger">
