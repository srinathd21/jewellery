...
if (!customerPermission($conn, 'open') && !customerPermission($conn, 'create')) {
...
$old = $_SESSION['customer_form_old'] ?? [];
unset($_SESSION['customer_form_old']);
$selectedServices = $old['services'] ?? ['Billing'];
...
<head>
    ...
    <title><?= e($businessName) ?> - Add Customer</title>
...
<body>
        ...
        <div class="content-wrap">
            ...
            <div class="page-head">
                <div>
                    ...
                    <div class="page-sub">Create one customer profile and enable Billing, Pawn and Chit services
                        independently.</div>
                </div><a href="customers.php" class="btn-soft text-decoration-none"><i
                        class="fa-solid fa-arrow-left me-1"></i>Customer List</a>
            </div>
            <form method="post" action="api/customers-save.php" id="customerForm" autocomplete="off">
                <input type="hidden" name="action" value="create"><input type="hidden" name="csrf_token"
                    value="<?= e($csrfToken) ?>">
                <div class="form-card">
                ...
                <div class="d-flex justify-content-end gap-2 mb-4"><button type="reset"
                        class="btn-soft">Reset</button><button type="submit" class="btn-theme" id="saveCustomer"><i
                            class="fa-solid fa-floppy-disk me-2"></i>Save Customer</button></div>
            </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            form.addEventListener('submit', function (e) { if (!checks.some(x => x.checked)) { e.preventDefault(); alert('Select at least one service: Billing, Pawn or Chit.'); return } const btn = document.getElementById('saveCustomer'); btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin me-2"></i>Saving...'; });
        });
    </script>
</body>
