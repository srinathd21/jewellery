...
if(empty($_SESSION['pawn_csrf']))$_SESSION['pawn_csrf']=bin2hex(random_bytes(32));$csrfToken=(string)$_SESSION['pawn_csrf'];
...
$stmt=$conn->prepare('SELECT * FROM business_theme_settings WHERE business_id=? LIMIT 1');if($stmt){$stmt->bind_param('i',$businessId);$stmt->execute();$r=$stmt->get_result()->fetch_assoc()?:[];$stmt->close();foreach($theme as $k=>$v)if(isset($r[$k])&&$r[$k]!=='')$theme[$k]=$r[$k];}
