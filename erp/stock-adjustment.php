...
if ($prodHasTaxType) {
    ...
    $productSql .= ", CASE WHEN COALESCE(tax_percent,0)>0 THEN 'GST' ELSE 'Non-GST' END AS tax_type";
} else {
    $productSql .= ", 'GST' AS tax_type";
}
...
if ($taxTypeFilter === 'GST') {
    if ($prodHasTaxType) {
        $productSql .= " AND COALESCE(tax_type,'GST')='GST'";
    } elseif ($prodHasTaxPercent) {
        $productSql .= " AND COALESCE(tax_percent,0)>0";
    }
} elseif ($taxTypeFilter === 'Non-GST') {
    if ($prodHasTaxType) {
        $productSql .= " AND COALESCE(tax_type,'GST')='Non-GST'";
    } elseif ($prodHasTaxPercent) {
        $productSql .= " AND COALESCE(tax_percent,0)<=0";
    } else {
        $productSql .= " AND 1=0";
...
if ($selectedProductId > 0) {
    $selectedTaxSelect = $prodHasTaxType
    ...
    $selectedTaxWhere = '';
    if ($taxTypeFilter === 'GST') {
        $selectedTaxWhere = $prodHasTaxType
            ? " AND COALESCE(p.tax_type,'GST')='GST'"
            : ($prodHasTaxPercent ? " AND COALESCE(p.tax_percent,0)>0" : '');
    } elseif ($taxTypeFilter === 'Non-GST') {
        $selectedTaxWhere = $prodHasTaxType
            ? " AND COALESCE(p.tax_type,'GST')='Non-GST'"
...
if ($taxTypeFilter === 'GST') {
    if ($prodHasTaxType) {
        $whereRecent .= " AND COALESCE(p.tax_type,'GST')='GST' ";
    } elseif ($prodHasTaxPercent) {
        $whereRecent .= " AND COALESCE(p.tax_percent,0)>0 ";
    }
} elseif ($taxTypeFilter === 'Non-GST') {
    if ($prodHasTaxType) {
        $whereRecent .= " AND COALESCE(p.tax_type,'GST')='Non-GST' ";
    } elseif ($prodHasTaxPercent) {
        $whereRecent .= " AND COALESCE(p.tax_percent,0)<=0 ";
    } else {
        $whereRecent .= " AND 1=0 ";
                                ...
                                <div class="mb-3"><b><?php echo h($selectedProduct['product_name']); ?></b>
                                    <div class="muted">
                                        <?php echo h($selectedProduct['product_code'] ?? ''); ?>        <?php if (!empty($selectedProduct['barcode'])): ?>
                                            ...
                                            · Purity <?php echo h($selectedProduct['purity']); ?>%<?php endif; ?>
                                        <?php if ($nonGstModeActive): ?>
                                            · <?= h($selectedProduct['tax_type'] ?? 'GST') ?>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="stock-detail">
                ...
                form.addEventListener('submit', async e => {
                    ...
                    try {
                        ...
                        setTimeout(() => {
                            ...
                            params.set('product_id', j.product_id || product.value);
                            <?php if ($nonGstModeActive): ?>
                            params.set('tax_type', <?php echo json_encode($taxTypeFilter); ?>);
                            <?php endif; ?>
                            location.href = 'stock-adjustment.php?' + params.toString();
                        }, 450);
                    } catch (err) {
