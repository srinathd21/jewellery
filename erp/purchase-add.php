...
function generateSupplierPaymentNo(mysqli $conn, int $businessId, int $branchId, string $paymentDate): string
{
    $period = date('Ym', strtotime($paymentDate));
    $prefix = 'SPY/' . $period . '/';
    $like = $prefix . '%';
    $last = '';

    $stmt = $conn->prepare("SELECT payment_no FROM supplier_payments WHERE business_id=? AND branch_id=? AND payment_no LIKE ? ORDER BY id DESC LIMIT 1");
    if ($stmt) {
        $stmt->bind_param('iis', $businessId, $branchId, $like);
        $stmt->execute();
        $row = $stmt->get_result()->fetch_assoc();
        $last = (string)($row['payment_no'] ?? '');
        $stmt->close();
    }
...
$supplierId = 0;
$invoiceNo = '';
$paymentSplits = [['method_id'=>0,'amount'=>'0.00','reference_no'=>'','remarks'=>'']];
$notes = '';
$discountAmount = '0.00';
$roundOff = '0.00';
$paidAmount = '0.00';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $postedPaymentMethods = (array)($_POST['payment_method_id'] ?? []);
    $postedPaymentAmounts = (array)($_POST['payment_amount'] ?? []);
    $postedPaymentReferences = (array)($_POST['payment_reference'] ?? []);
    $postedPaymentRemarks = (array)($_POST['payment_remarks'] ?? []);
    $paymentSplits = [];
    $paidAmountF = 0.00;

    foreach ($postedPaymentMethods as $paymentIndex => $methodRaw) {
        $methodId = (int)$methodRaw;
        $splitAmount = round((float)($postedPaymentAmounts[$paymentIndex] ?? 0), 2);
        $referenceNo = trim((string)($postedPaymentReferences[$paymentIndex] ?? ''));
        $splitRemarks = trim((string)($postedPaymentRemarks[$paymentIndex] ?? ''));

        if ($methodId <= 0 && $splitAmount <= 0) {
            continue;
        }

        $paymentSplits[] = [
            'method_id' => $methodId,
            'amount' => moneyf($splitAmount),
            'reference_no' => $referenceNo,
            'remarks' => $splitRemarks,
        ];
        $paidAmountF += $splitAmount;
    }
    ...
    if ($purchaseDate === '') {
        ...
        foreach ($paymentSplits as $split) {
            if ((int)$split['method_id'] <= 0 && (float)$split['amount'] > 0) {
                $error = 'Select a payment method for every split amount.';
                break;
            }
            if ((int)$split['method_id'] > 0 && (float)$split['amount'] <= 0) {
                $error = 'Enter an amount for every selected payment method.';
                break;
            }
        }
        ...
        if ($error === '') {
            ...
            $paidAmountF = round((float)$paidAmount, 2);

            $cgstAmount = round($gstTotal / 2, 2);
            $sgstAmount = round($gstTotal / 2, 2);
            $igstAmount = 0.00;
            $grandTotal = $taxableTotal + $gstTotal + $roundOffF;
            if ($paidAmountF > $grandTotal + 0.01) {
                $error = 'Split payment total cannot exceed the purchase grand total.';
            }
            ...
            try {
                ...
                if ($paidAmountF > 0) {
                    if (!tableExists($conn, 'supplier_payments')) {
                        throw new Exception('supplier_payments table is required to record the purchase payment.');
                    }

                    $validMethodIds = [];
                    $methodNameMap = [];
                    $methodStmt = $conn->prepare("SELECT id,method_name FROM payment_methods WHERE id=? AND business_id=? AND is_active=1 LIMIT 1");
                    if (!$methodStmt) {
                        throw new Exception('Unable to validate payment methods: ' . $conn->error);
                    }

                    foreach ($paymentSplits as $split) {
                        $methodIdValue = (int)$split['method_id'];
                        $methodStmt->bind_param('ii', $methodIdValue, $businessId);
                        $methodStmt->execute();
                        $methodRow = $methodStmt->get_result()->fetch_assoc();
                        if (!$methodRow) {
                            throw new Exception('One of the selected payment methods is invalid or inactive.');
                        }
                        $validMethodIds[] = $methodIdValue;
                        $methodNameMap[$methodIdValue] = (string)$methodRow['method_name'];
                    }
                    $methodStmt->close();

                    $paymentNo = generateSupplierPaymentNo($conn, $businessId, $branchId, $purchaseDate);
                    $primaryMethodId = (int)$validMethodIds[0];
                    $primaryReference = (string)($paymentSplits[0]['reference_no'] ?? '');
                    $paymentRemarks = $notes !== '' ? $notes : ('Payment recorded while creating purchase ' . $purchaseNo);

                    $supplierPaymentStmt = $conn->prepare(
                        "INSERT INTO supplier_payments
                        (business_id,branch_id,supplier_id,purchase_id,payment_no,payment_date,
                         payment_method_id,amount,reference_no,remarks,created_by)
                        VALUES (?,?,?,?,?,?,?,?,?,?,?)"
                    );
                    if (!$supplierPaymentStmt) {
                        throw new Exception('Unable to prepare supplier payment: ' . $conn->error);
                    }
                    $supplierPaymentStmt->bind_param(
                        'iiiissidssi',
                        $businessId,
                        $branchId,
                        $supplierId,
                    ...
                    if (!$supplierPaymentStmt->execute()) {
                        throw new Exception('Unable to save supplier payment: ' . $supplierPaymentStmt->error);
                    }
                    $supplierPaymentId = (int)$supplierPaymentStmt->insert_id;
                    $supplierPaymentStmt->close();
                    ...
                    if (tableExists($conn, 'supplier_payment_splits')) {
                        ...
                        if (!$splitStmt) {
                            throw new Exception('Unable to prepare supplier payment split: ' . $conn->error);
                        }
                        foreach ($paymentSplits as $split) {
                            $methodIdValue = (int)$split['method_id'];
                            $methodNameValue = $methodNameMap[$methodIdValue] ?? 'Other';
                            $allowedSplitNames = ['Cash','UPI','Bank Transfer','Card','Cheque','Other'];
                            if (!in_array($methodNameValue, $allowedSplitNames, true)) {
                                $methodNameValue = 'Other';
                            }
                            $splitAmountValue = (float)$split['amount'];
                            $splitReferenceValue = (string)$split['reference_no'];
                            $splitRemarksValue = (string)$split['remarks'];
                            $splitStmt->bind_param('isdss', $supplierPaymentId, $methodNameValue, $splitAmountValue, $splitReferenceValue, $splitRemarksValue);
                            if (!$splitStmt->execute()) {
                                throw new Exception('Unable to save supplier payment split: ' . $splitStmt->error);
                            }
                        }
                        $splitStmt->close();
                    }
                    ...
                    if (tableExists($conn, 'supplier_payment_allocations')) {
                        ...
                        if (!$allocationStmt) {
                            throw new Exception('Unable to prepare supplier payment allocation: ' . $conn->error);
                        }
                        $allocationStmt->bind_param('iid', $supplierPaymentId, $purchaseId, $paidAmountF);
                        if (!$allocationStmt->execute()) {
                            throw new Exception('Unable to save supplier payment allocation: ' . $allocationStmt->error);
                        }
                    ...
                    <section class="section-card">
                        <div class="section-head">
                            <div>
                                ...
                                <div class="section-subtitle">Split the paid amount across Cash, UPI or other active payment methods. Each split is saved in the supplier payment ledger.</div>
                            </div>
                            <button type="button" class="btn-soft" id="addPaymentSplitBtn"><i class="fa-solid fa-plus me-1"></i>Add Method</button>
                        </div>
                        <div class="section-body">
                            <div id="paymentSplitRows">
                                <?php foreach ($paymentSplits as $splitIndex => $split): ?>
                                <div class="payment-split-row row g-2 mb-2 align-items-end">
                                    <div class="col-lg-3 col-md-6">
                                        <label class="form-label">Payment Method</label>
                                        <select name="payment_method_id[]" class="form-select split-method">
                                            <option value="">Select Payment Method</option>
                                            <?php foreach ($paymentMethods as $pm): ?>
                                                <option value="<?php echo (int)$pm['id']; ?>" <?php echo (int)$split['method_id'] === (int)$pm['id'] ? 'selected' : ''; ?>><?php echo h($pm['method_name']); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="col-lg-2 col-md-6">
                                        <label class="form-label">Amount</label>
                                        <input type="number" step="0.01" min="0" name="payment_amount[]" class="form-control split-payment-amount" value="<?php echo h($split['amount']); ?>">
                                    </div>
                                    <div class="col-lg-3 col-md-6">
                                        <label class="form-label">Reference / UTR</label>
                                        <input type="text" name="payment_reference[]" class="form-control" value="<?php echo h($split['reference_no']); ?>" maxlength="120">
                                    </div>
                                    <div class="col-lg-3 col-md-6">
                                        <label class="form-label">Payment Remarks</label>
                                        <input type="text" name="payment_remarks[]" class="form-control" value="<?php echo h($split['remarks']); ?>" maxlength="200">
                                    </div>
                                    <div class="col-lg-1 col-md-12">
                                        <button type="button" class="remove-payment-split remove-row" title="Remove method"><i class="fa-solid fa-trash"></i></button>
                                    </div>
                                </div>
                                <?php endforeach; ?>
                            </div>
                            <?php if (!$paymentMethods): ?>
                                <div class="alert alert-warning modern-alert mt-2">No active payment methods are available for this business.</div>
                            <?php endif; ?>
                            <div class="row g-3 mt-1">
                                <div class="col-12">
                                    <label class="form-label">Purchase Notes</label>
                                    <input type="text" name="notes" class="form-control" value="<?php echo h($notes); ?>" placeholder="Optional purchase notes">
                                </div>
                            </div>
                        </div>
                    </section>
...
(function () {
    ...
    function paymentMethodOptions() {
        ...
        paymentMethods.forEach(function (method) {
            html += '<option value="' + Number(method.id) + '">' + escapeHtml(method.method_name) + '</option>';
        });
        return html;
    }

    function addPaymentSplitRow() {
        const wrap = document.getElementById('paymentSplitRows');
        const row = document.createElement('div');
        row.className = 'payment-split-row row g-2 mb-2 align-items-end';
        row.innerHTML = `
            <div class="col-lg-3 col-md-6"><label class="form-label">Payment Method</label><select name="payment_method_id[]" class="form-select split-method">${paymentMethodOptions()}</select></div>
            <div class="col-lg-2 col-md-6"><label class="form-label">Amount</label><input type="number" step="0.01" min="0" name="payment_amount[]" class="form-control split-payment-amount" value="0.00"></div>
            <div class="col-lg-3 col-md-6"><label class="form-label">Reference / UTR</label><input type="text" name="payment_reference[]" class="form-control" maxlength="120"></div>
            <div class="col-lg-3 col-md-6"><label class="form-label">Payment Remarks</label><input type="text" name="payment_remarks[]" class="form-control" maxlength="200"></div>
            <div class="col-lg-1 col-md-12"><button type="button" class="remove-payment-split remove-row" title="Remove method"><i class="fa-solid fa-trash"></i></button></div>`;
        wrap.appendChild(row);
        row.querySelector('.split-payment-amount').addEventListener('input', calculateAll);
    }

    function splitPaidTotal() {
        let total = 0;
        document.querySelectorAll('.split-payment-amount').forEach(function (input) {
    ...
    function calculateAll() {
        ...
        const paidAmount = splitPaidTotal();
    ...
    document.getElementById('addPaymentSplitBtn')?.addEventListener('click', addPaymentSplitRow);
    document.querySelectorAll('.split-payment-amount').forEach(function (input) {
        input.addEventListener('input', calculateAll);
    });
    document.addEventListener('click', function (event) {
        const button = event.target.closest('.remove-payment-split');
        if (!button) return;
        const rows = document.querySelectorAll('.payment-split-row');
        if (rows.length <= 1) {
            showToast('warning', 'At least one payment row must remain. Leave it blank for an unpaid purchase.');
            return;
        }
    ...
    function buildPurchasePreview(){
        ...
        let paymentError = '';
        document.querySelectorAll('.payment-split-row').forEach(function(row){
            const method = row.querySelector('.split-method')?.value || '';
            const amount = parseFloat(row.querySelector('.split-payment-amount')?.value || 0);
            if (!paymentError && amount > 0 && !method) paymentError = 'Select a payment method for every split amount.';
            if (!paymentError && method && amount <= 0) paymentError = 'Enter an amount for every selected payment method.';
        });
        if (paymentError) { showToast('error', paymentError); return false; }
        if (splitPaidTotal() > parseFloat(document.getElementById('grand_total').value || 0) + 0.01) {
        ...
        document.getElementById('purchasePreviewBody').innerHTML=`
            ...
            <div class="preview-total-panel">
                ...
                <div class="preview-total-row"><span>Paid (Split Total)</span><strong><?php echo h($currencySymbol); ?>${splitPaidTotal().toFixed(2)}</strong></div>
